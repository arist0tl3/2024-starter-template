...
export default async function getUser(
  ...
  try {
    ...
    // Forged token?
    if (!decoded?.phoneNumber) throw new Error("Unable to verify this token");

    // Somehow passing the wrong token?
    if (!decoded?.type) throw new Error("Unable to verify this token");
    if (decoded?.type !== "auth")
      throw new Error("Unable to verify this token");
